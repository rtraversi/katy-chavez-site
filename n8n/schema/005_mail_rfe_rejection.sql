-- KCL Portal — Phase 3b: RFE + rejection fields for mail_items
--
-- Apply on the VPS:
--   cat /opt/kcl-repo/n8n/schema/005_mail_rfe_rejection.sql | \
--     sudo docker compose -f /opt/kcl-n8n/docker-compose.yml exec -T \
--       postgres psql -U portal -d kcl_portal
--
-- Re-runnable: ALTER TABLE uses IF NOT EXISTS; function uses CREATE OR REPLACE.

-- ── New columns on mail_items ──────────────────────────────────────
ALTER TABLE mail_items
  ADD COLUMN IF NOT EXISTS response_due_date       date,         -- RFE deadline
  ADD COLUMN IF NOT EXISTS rfe_evidence_requested  text[],       -- list of items Claude extracted
  ADD COLUMN IF NOT EXISTS rejection_reason        text;         -- why USCIS returned the package

-- ── Update mail_intake to persist new fields ───────────────────────
